...
CREATE TABLE links (
    ...
    kind TEXT NOT NULL CHECK(kind IN ('redirect', 'collection')),
    target_url TEXT,
    user_id BIGINT,
    password_hash TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE OR REPLACE FUNCTION links_set_kind_from_target_url()
RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER trg_links_set_kind
BEFORE INSERT OR UPDATE OF target_url ON links
FOR EACH ROW
EXECUTE FUNCTION links_set_kind_from_target_url();
